...
void __kException(DWORD param) {
	...
	{
		__drawGraphChars((unsigned char*)showinfo, EXCEPTION_TIPS_COLOR);
		...
		if (taskinfo)
		{
			__printf(showinfo, "task pid:%d,level:%d,status:%x,counter:%d,entry:%x,delay:%d,function:%s,file:%s\n",
				taskinfo->tid, taskinfo->level, taskinfo->status,
				taskinfo->counter, taskinfo->moduleaddr,
				0, taskinfo->funcname, taskinfo->filename);
			__drawGraphChars((unsigned char*)showinfo, EXCEPTION_TIPS_COLOR);
		}
	}
	...
	if (tid == process->pid)
	{
		__terminateProcess(process->pid | 0x80000000, process->filename, process->funcname, 0);
	}
	else {
		__kTerminateThread(process->tid | 0x80000000, process->filename,process->funcname, 0);
	}
