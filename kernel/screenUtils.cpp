...
extern "C" __declspec(dllexport) int __kPrintScreen() {
	...
	BITMAPFILEHEADER * hdr = (BITMAPFILEHEADER*)FILE_BUFFER_ADDRESS;
	hdr->bfType = 0x4d42;
	...
	BITMAPINFOHEADER * info = (BITMAPINFOHEADER*)(FILE_BUFFER_ADDRESS + sizeof(BITMAPFILEHEADER));
	...
	__memcpy((char*)(FILE_BUFFER_ADDRESS + sizeof(BITMAPFILEHEADER) + sizeof(BITMAPINFOHEADER)),
		(char*)gGraphBase, screensize);

	char filename[256];
	__sprintf(filename, "c:\\%x.bmp",*(unsigned int*)TIMER0_TICK_COUNT);
	int ret = writeFile(filename, (char*)FILE_BUFFER_ADDRESS, screensize + sizeof(BITMAPFILEHEADER) + sizeof(BITMAPINFOHEADER),FILE_WRITE_COVER);
