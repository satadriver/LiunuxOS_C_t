...
int mainProc() {
	...
	ret = DeviceWriter::dataReader(0, SECTOR_SIZE, mbr);
	if (ret <= 0)
	{
	...
	char dbr[1024] = { 0 };
	ret = DeviceWriter::dataReader(lpmbr->dpt[0].offset, SECTOR_SIZE, dbr);
	if (ret <= 0)
	{
	...
	int endsecno = 0;
	int startsecoff = 0;
	if (lpmbr->dpt[0].type == FAT32_PARTITION 
		|| lpmbr->dpt[0].type == FAT32_PARTITION_2 
		|| lpmbr->dpt[0].type == FAT32_LBA_PARTITION
		|| lpmbr->dpt[0].type == FAT32_HIDDEN)
	{
		

		LPFAT32_DBR fat32dbr = (LPFAT32_DBR)dbr;
		if (fat32dbr->BPB_HiddSec > EMPTY_SECTOR_NEED + 256 )
		{
			startsecoff = fat32dbr->BPB_HiddSec - EMPTY_SECTOR_NEED - 256;
			endsecno = fat32dbr->BPB_HiddSec - EMPTY_SECTOR_NEED ;
		}else if (fat32dbr->BPB_RsvdSecCnt > EMPTY_SECTOR_NEED + 256)
		{
			startsecoff = fat32dbr->BPB_HiddSec + fat32dbr->BPB_RsvdSecCnt - EMPTY_SECTOR_NEED - 256;
			endsecno = fat32dbr->BPB_HiddSec + fat32dbr->BPB_RsvdSecCnt - EMPTY_SECTOR_NEED;
		}
		else {
			
// 			startsecoff = fat32dbr->BPB_TotSec32 - EMPTY_SECTOR_NEED * 4;
// 			endsecno = fat32dbr->BPB_TotSec32 - EMPTY_SECTOR_NEED;

			DWORD rootdir = fat32dbr->BPB_HiddSec + fat32dbr->BPB_RsvdSecCnt + (fat32dbr->BPB_FATSz32*fat32dbr->BPB_NumFATs);
			startsecoff = rootdir - EMPTY_SECTOR_NEED * 4;
			endsecno = rootdir - EMPTY_SECTOR_NEED;
		}
	}else if (lpmbr->dpt[0].type == NTFS_PARTITION || lpmbr->dpt[0].type == NTFS_HIDDEN)
	{
		LPNTFSDBR ntfsdbr = (LPNTFSDBR)dbr;
		startsecoff = ntfsdbr->hideSectors -  EMPTY_SECTOR_NEED - 256;
		if (startsecoff <= 0)
		{
			wsprintfA(szout, "hide sector:%d,start sector:%d error", ntfsdbr->hideSectors, startsecoff);
			MessageBoxA(0, szout, szout, MB_OK);
			//return FALSE;
			startsecoff = ntfsdbr->hideSectors + ntfsdbr->MFT*ntfsdbr->secPerCluster - 4 * EMPTY_SECTOR_NEED;
			//startsecoff = ntfsdbr->hideSectors+16;
			//startsecoff = lpmbr->dpt[0].sectortotal + lpmbr->dpt[0].offset - 16 * EMPTY_SECTOR_NEED;
			endsecno = startsecoff +  EMPTY_SECTOR_NEED;
		}
		else {
			endsecno = ntfsdbr->hideSectors - EMPTY_SECTOR_NEED;
		}
	}
	else {
		wsprintfA(szout, "start sec number:%d,end sector number:%d error\n", startsecoff, endsecno);
		MessageBoxA(0, szout, szout, MB_OK);
		return FALSE;
	}

	wsprintfA(szout, "start sec number:%d,end sector number:%d\n", startsecoff, endsecno);
	MessageBoxA(0, szout, szout, MB_OK);
...
// 	for (; freesecno < endsecno; freesecno++)
// 	{
// 		ret = DeviceWriter::dataReader(freesecno, SECTOR_SIZE *EMPTY_SECTOR_NEED, buf);
// 		if (ret <= 0)
// 		{
// 			return FALSE;
// 		}
...
// 	}
...
#ifdef _DEBUG
	ret = DeviceWriter::dataReader(freesecno, SECTOR_SIZE, buf);
	strcpy(buf, "hello,how are you?fine thank you,and you?i'm fine too");
	ret = DeviceWriter::dataWriter(freesecno, SECTOR_SIZE, buf);
	...
	ret = DeviceWriter::dataWriter(0, SECTOR_SIZE, mymbr);
	if (ret <= 0)
	{
	...
	if (flag == 1)
	{
		...
		ret = DeviceWriter::dataWriter(hdr->mbrSecOff, SECTOR_SIZE, mbr);
		if (ret <= 0)
		{
		...
		ret = DeviceWriter::dataWriter(hdr->mbr2SecOff, SECTOR_SIZE, mbr);
		if (ret <= 0)
		{
	...
	ret = DeviceWriter::dataWriter(freesecno, SECTOR_SIZE, (char*)hdr);
	if (ret <= 0)
	{
	...
	ret = DeviceWriter::dataWriter(freesecno + 3, SECTOR_SIZE * 2, (char*)fontbuf);
	if (ret <= 0)
	{
	...
	ret = DeviceWriter::dataWriter(hdr->loaderSecOff, loadersize, (char*)loaderbuf);
	if (ret <= 0)
	{
	...
	ret = DeviceWriter::dataWriter(hdr->kernelSecOff,kersize, (char*)kerbuf);
	if (ret <= 0)
	{
	...
	ret = DeviceWriter::dataWriter(hdr->kerdllSecOff, kerdllsize, (char*)kerdllbuf);
	if (ret <= 0)
	{
	...
	ret = DeviceWriter::dataWriter(hdr->maindllSecOff, maindllsize, (char*)maindllbuf);
	if (ret <= 0)
	{
