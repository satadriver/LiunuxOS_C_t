...
int DeviceWriter::sectorWriter(const char *name, unsigned int sectoroffset, char * data) {
	int ret = 0;
...
int DeviceWriter::sectorReader(const char *name, unsigned int sectoroffset, int cnt, char * data) {
	int ret = 0;
...
int DeviceWriter::dataReader(unsigned int sectoroffset, int readsize, char * data) {
	int ret = 0;
...
//不能一次性写入超过64kb的数据？
int DeviceWriter::dataWriter(unsigned int sectoroffset, int datasize, char * data) {
#ifdef _DEBUG
	return TRUE;
#endif
	...
	if (writeblocksize)
	{
		int blockcnt = writeblocksize / 0x10000;

		for (int i = 0; i < blockcnt; i++)
		{
			dwRet = 0;
			ret = WriteFile(g_hDev, data + i * 0x10000, 0x10000, &dwRet, 0);
			if (ret <= 0)
			{
				ret = GetLastError();
				return FALSE;
		...
		int blockmod = writeblocksize % 0x10000;
		if (blockmod)
		{
			ret = WriteFile(g_hDev, data + blockcnt * 0x10000, blockmod, &dwRet, 0);
			if (ret <= 0)
			{
				ret = GetLastError();
				return FALSE;
