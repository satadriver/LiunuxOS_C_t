...
int __cmd(char * cmd,WINDOWCLASS* window,char * pidname,int pid) {
	...
	if (__strcmp(params[0], "dump") == 0 && paramcnt >= 2)
	{
		...
		if (__memcmp(filename + fnlen - 4, ".bmp", 4) == 0)
		{
			...
			return __kCreateProcess(VSMAINDLL_LOAD_ADDRESS,0x20000, "main.dll", "__kShowWindow", 3, (DWORD)&taskcmd);
		}
		...
		{
			//dump filename level entry param
			int level = 3;
			if (paramcnt >= 3)
			{
				level = __strd2i(params[2]);
				if (paramcnt >= 4)
				{
					if (paramcnt >= 5)
					{
						return __kCreateProcess(0, 0, filename, params[3], level, (DWORD)params[4]);
					}
					else {
						return __kCreateProcess(0, 0, filename, params[3], level, 0);
					}
				}
				else {
					return __kCreateProcess(0, 0, filename, filename, level, 0);
				}
			}
			else {
				return __kCreateProcess(0, 0,filename, filename, level, 0);
		...
		else if (__memcmp(filename + fnlen - 3, ".so", 3) == 0)
		{
	...
	else if (__strcmp(params[0], "run") == 0)
	{
		if (paramcnt >= 2 && __strcmp(params[1], "paint") == 0)
		{
			...
			return __kCreateProcess(VSMAINDLL_LOAD_ADDRESS, 0x20000, "main.dll", "__kPaint", 3, (DWORD)&taskcmd);
		}
	}
