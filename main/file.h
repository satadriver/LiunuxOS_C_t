...
#define FILE_WRITECREATE_OPEN	1
#define FOLDER_WRITE_CREATE		2
#define FILE_WRITE_APPEND		3
#define FILE_WRITE_COVER		4
...
extern "C" __declspec(dllexport)  int (__cdecl * writeFile)(char * filename, char * buf, int size, int createmode, int writemode);
#else
...
extern "C" __declspec(dllimport)  int(__cdecl * writeFile)(char * filename, char * buf, int size, int createmode, int writemode);
