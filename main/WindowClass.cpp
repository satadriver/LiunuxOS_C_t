...
int __kShowWindow(unsigned int retaddr, int tid, char * filename, char * funcname,DWORD runparam) {
	...
	if (cmd == SHOW_WINDOW_BMP || cmd == SHOW_WINDOW_TXT || cmd == SHOW_WINDOW_JPEG)
	{
		...
		{
			filesize = readFileTo(taskcmd->filename);
			if (filesize <= 0)
			{
			...
			filebuf = (char*)FILE_BUFFER_ADDRESS;
		}
		else {
		...
		else if (cmd == SHOW_WINDOW_TXT)
		{
			...
			*(data + filesize) = 0;
			...
			char * bmpdata = (char*)FILE_BUFFER_ADDRESS + FILE_BUFFER_SIZE - 0x1000000;
			int bmpsize = 0x1000000;
