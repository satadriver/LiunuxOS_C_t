...
int showTxtFile(char * filename) {
	...
	int ret = 0;
	int filesize = readFileTo(filename);
	unsigned char * data = (unsigned char*)FILE_BUFFER_ADDRESS;
...
int showBmpFile(char * filename) {
	...
	int filesize = readFileTo(filename);
	unsigned char * data = (unsigned char*)FILE_BUFFER_ADDRESS;
	return showBmp(filename, (unsigned char *)FILE_BUFFER_ADDRESS, filesize,0,0);
}
...
int playWavFile(char *filename) {
	...
	int filesize = readFileTo(filename);
	unsigned char * data = (unsigned char*)FILE_BUFFER_ADDRESS;
	return sbplay(( char *)FILE_BUFFER_ADDRESS, filesize);
}
